...
class SettingsManager:
    ...
    def __init__(self, settings_filename="iptv_settings.json"):
        """
    ...
    def _get_settings_filepath(self, filename):
        ...
        if os.name == 'nt': # Windows
            ...
            config_dir = os.path.join(app_data_dir, "PythonIPTVPlayer")
        else: # Linux, macOS
            xdg_config_home = os.getenv('XDG_CONFIG_HOME')
            if xdg_config_home:
                config_dir = os.path.join(xdg_config_home, "PythonIPTVPlayer")
            else:
                # Fallback to ~/.config/PythonIPTVPlayer
                config_dir = os.path.join(os.path.expanduser("~"), ".config", "PythonIPTVPlayer")
    ...
    def load_settings(self):
        ...
        if os.path.exists(self.settings_filepath):
            try:
                ...
                print(f"Warning: Could not decode JSON from {self.settings_filepath}. Using default settings.")
                ...
                try:
                    self.save_settings()
                    print(f"Created new clean settings file at {self.settings_filepath}")
                except Exception as save_error:
                    print(f"Could not save clean settings: {save_error}")
            except Exception as e:
                print(f"Error loading settings from {self.settings_filepath}: {e}. Using default settings.")
                self.settings = self.DEFAULT_SETTINGS.copy()
        else:
            print(f"Settings file not found at {self.settings_filepath}. Using default settings and creating a new one on save.")
            self.settings = self.DEFAULT_SETTINGS.copy() # Ensure defaults are used if file doesn't exist
...
if __name__ == '__main__':
    ...
    try:
        if os.path.exists(manager.settings_filepath): # Use the path from the first manager
            ...
            if "PythonIPTVPlayer" in config_dir and os.path.exists(config_dir) and not os.listdir(config_dir):
                os.rmdir(config_dir)
