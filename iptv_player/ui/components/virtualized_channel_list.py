from PySide6.QtWidgets import (QAbstractItemView, QWidget, QVBoxLayout, QScrollBar,
                             QFrame, QLabel, QLineEdit, QHBoxLayout, QPushButton)
...
class VirtualizedChannelList(QWidget):
    ...
    def setup_ui(self):
        ...
        self.search_input.setPlaceholderText("Search channels by name, category, or ID... (Ctrl+F to focus)")
    ...
    def _update_info_label(self):
        ...
        if total == 0:
            ...
            self.info_label.setText(f"Showing {filtered:,} of {total:,} channels (filtered)")
        else:
    ...
    def _move_selection(self, delta):
        ...
        new_index = max(0, min(len(self._filtered_channels) - 1, self.selected_index + delta))
...
class ChannelViewport(QFrame):
    ...
    def paintEvent(self, event):
        ...
        render_end = min(len(channels), start_idx + visible_count + self.parent_list.render_buffer)
        ...
        for i in range(render_start, render_end):
            ...
            elided_text = font_metrics.elidedText(display_text, Qt.ElideRight, text_rect.width())
