...
class QMediaVideoPlayer(QObject):
    ...
    def __init__(self, video_widget=None):
        ...
        try:
            import os
            # Use software decoding for better compatibility
            os.environ['QT_MULTIMEDIA_FFMPEG_HWACCEL'] = 'none'
        except:
            pass
        ...
        # Configure audio output for AC3 support
        # Audio format is handled automatically by Qt's media framework
        # The environment variables set earlier will configure ffmpeg for AC3
    ...
    def _extract_metadata(self):
        ...
        try:
            ...
            if hasattr(meta_data, 'value'):
                ...
                try:
                    ...
                    if hasattr(QMediaMetaData.Key, 'Resolution'):
                        try:
                            ...
                            if resolution and hasattr(resolution, 'width') and hasattr(resolution, 'height'):
                                metadata['resolution'] = f"{resolution.width()}x{resolution.height()}"
                        except Exception:
                        ...
                        try:
                            ...
                            if audio_codec:
                                try:
                                    ...
                                    metadata['audio_codec'] = "AC3"  # Assume AC3 if unknown
                        except Exception:
                            pass
                    ...
                    if hasattr(QMediaMetaData.Key, 'VideoBitRate'):
                        try:
                            video_bitrate = meta_data.value(QMediaMetaData.Key.VideoBitRate)
                            if video_bitrate:
                                metadata['video_bitrate'] = f"{int(video_bitrate) // 1000} kbps"
                        except Exception:
                            pass
                    ...
                    if hasattr(QMediaMetaData.Key, 'AudioBitRate'):
                        try:
                            audio_bitrate = meta_data.value(QMediaMetaData.Key.AudioBitRate)
                            if audio_bitrate:
                                metadata['audio_bitrate'] = f"{int(audio_bitrate) // 1000} kbps"
                        except Exception:
                            pass
                    ...
                    for attr_name in ['SampleRate', 'AudioSampleRate', 'SamplingRate']:
                        if hasattr(QMediaMetaData.Key, attr_name):
                            try:
                                sample_rate = meta_data.value(getattr(QMediaMetaData.Key, attr_name))
                                if sample_rate:
                                    break
                            except Exception:
                                continue
            ...
            if self.current_url and 'hevc' not in metadata.get('video_codec', '').lower():
            ...
            # Add audio codec if not detected (assume AC3 for IPTV streams)
